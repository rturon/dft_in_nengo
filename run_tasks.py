...
model, nodes, probes = create_model('./JSON/mental_imagery_extended.json',
...
def plot_probes(sim, probes, save=False):
    ...
    time_points = np.linspace(0,sim.data[probes['Indeterminent ']].shape[0]-1,36, dtype=int)

    if save:
        filename = '_Spatial Relation_%s.png' %timestamp
        plot_2d(sim.data[probes['Indeterminent ']], time_points, colorbar=True, 
                title=None, save=save+filename)
        
        filename = '_Reference Field_%s.png' %timestamp
        plot_2d(sim.data[probes['Reference']], time_points, colorbar=True, 
                title=None, save=save+filename)

        filename = '_Target Field_%s.png' %timestamp
        plot_2d(sim.data[probes['Target']], time_points, colorbar=True, 
                title=None, save=save+filename)

        filename = '_Relational Field_%s.png' %timestamp
        plot_2d(sim.data[probes['Object-centered ']], time_points, colorbar=True, 
                title=None, save=save+filename)

        filename = '_Colour_%s.png' %timestamp
        plot_1d(sim.data[probes['Colour']], time_points, title=None, save=save+filename)
