...
if SAVE_SIMULATION:
    ...
    for ob_key in probes:
        ...
        np.save('./simulation_data/%s/%s_%s' %(timestamp, file_name, timestamp), 
...
plots_1d = ['Target Red Memory', 'Target Blue Memory', 'Target Cyan Memory', 
...
# plot spatial scene       
filepath = '../images/%.2f/Spatial Scene_%.2f_%s.png' %(TAU_FACTOR, TAU_FACTOR, 
                                                        timestamp)
plot_2d(simdata[probes['Indeterminent ']], time_points, True, 
                   'Spatial Scene - Tau_factor %.2f' %TAU_FACTOR, filepath)
# plot sigmoided spatial scene
filepath = '../images/%.2f/Sigmoided Spatial Scene_%.2f_%s.png' %(TAU_FACTOR, TAU_FACTOR, 
                                                        timestamp)
plot_2d(sigmoid(simdata[probes['Indeterminent ']]), time_points, 
                   True, 'Sigmoided Spatial Scene - Tau_factor %.2f' %TAU_FACTOR, 
                   filepath)
# plot the colour field
filepath = '../images/%.2f/Colour_%.2f_%s.png' %(TAU_FACTOR, TAU_FACTOR, 
                                                        timestamp)
plot_1d(simdata[probes['Colour']], time_points, 'Colour - Tau_factor %.2f' %TAU_FACTOR, 
        filepath)
# plot the relational field
filepath = '../images/%.2f/Relational Field_%.2f_%s.png' %(TAU_FACTOR, TAU_FACTOR, 
                                                        timestamp)
object_centered_data = simdata[probes['Object-centered ']]
plot_2d(object_centered_data, time_points, True, 'Relational Field - Tau_factor %.2f' %TAU_FACTOR, 
        filepath)
# plot the target field
filepath = '../images/%.2f/Target Field_%.2f_%s.png' %(TAU_FACTOR, TAU_FACTOR, 
                                                        timestamp)
target = simdata[probes['Target']]
plot_2d(target, time_points, True, 'Target Field - Tau_factor %.2f' %TAU_FACTOR, 
        filepath)
# plot the reference field
filepath = '../images/%.2f/Reference Field_%.2f_%s.png' %(TAU_FACTOR, TAU_FACTOR, 
                                                        timestamp)
reference_data = simdata[probes['Reference']]
plot_2d(reference_data, time_points, True, 'Reference Field - Tau_factor %.2f' %TAU_FACTOR, 
        filepath)
